...
import io.trino.plugin.datainsight.ptf.RawQuery.RawQueryFunctionHandle;
import io.trino.plugin.datainsight.ptf.Query.QueryFunctionHandle;
...
import static io.trino.plugin.datainsight.DatainsightTableHandle.Type.*;
...
public class DatainsightMetadata
    ...
    // See https://www.elastic.co/guide/en/datainsight/reference/current/regexp-syntax.html
    ...
    @Inject
    public DatainsightMetadata(TypeManager typeManager, DatainsightClient client, DatainsightConfig config) {
    ...
    @Override
    public DatainsightTableHandle getTableHandle(ConnectorSession session, SchemaTableName tableName) {
        ...
        if (tableName.getSchemaName().equals(schemaName)) {
            ...
            if (client.indexExists(table) && !client.getIndexMetadata(table).getSchema().getFields().isEmpty()) {
                return new DatainsightTableHandle(SCAN, schemaName, table, query);
    ...
    public ConnectorTableMetadata getTableMetadata(ConnectorSession session, ConnectorTableHandle table) {
        DatainsightTableHandle handle = (DatainsightTableHandle) table;
    ...
    private Map<String, ColumnHandle> makeColumnHandles(List<IndexMetadata.Field> fields) {
        ...
        for (IndexMetadata.Field field : fields) {
            ...
            result.put(field.getName(), new DatainsightColumnHandle(
                    field.getName(),
                    converted.getType(),
                    0,
    ...
    private TypeAndDecoder toTrino(String prefix, IndexMetadata.Field field) {
        ...
        if (type instanceof IndexMetadata.PrimitiveType primitiveType) {
        ...
        } else if (type instanceof IndexMetadata.ScaledFloatType) {
            return new TypeAndDecoder(DOUBLE, new DoubleDecoder.Descriptor(path));
        } else if (type instanceof IndexMetadata.DateTimeType dateTimeType) {
...
//            return new TypeAndDecoder(TIMESTAMP_MILLIS, new TimestampDecoder.Descriptor(path, formats));
        } else if (type instanceof IndexMetadata.ObjectType objectType) {
    ...
    public Map<String, ColumnHandle> getColumnHandles(ConnectorSession session, ConnectorTableHandle tableHandle) {
        DatainsightTableHandle table = (DatainsightTableHandle) tableHandle;

        if (isPassthroughQuery(table)) {
            return PASSTHROUGH_QUERY_COLUMNS;
        }
        if (isPluginQuery(table)) {
            DatainsightRecord pluginResult = getPluginResult(table.getType().toString(), table.getQuery().get());
            Map<String, ColumnHandle> columns = new HashMap<>();
            List<Map<String, String>> resultColumns = pluginResult.getColumns();
            for (int i = 0; i < resultColumns.size(); i++) {
                columns.put(resultColumns.get(i).get("name"), new DatainsightColumnHandle(
                        resultColumns.get(i).get("name"),
                        getColumnType(resultColumns.get(i).get("type")),
                        i,
                        null,
                        false));
            }
    ...
    public ColumnMetadata getColumnMetadata(ConnectorSession session, ConnectorTableHandle tableHandle, ColumnHandle columnHandle) {
        DatainsightTableHandle table = (DatainsightTableHandle) tableHandle;
        DatainsightColumnHandle column = (DatainsightColumnHandle) columnHandle;
    ...
    public Optional<LimitApplicationResult<ConnectorTableHandle>> applyLimit(ConnectorSession session, ConnectorTableHandle table, long limit) {
        ...
        handle = new DatainsightTableHandle(
                handle.getType(),
                handle.getSchema(),
                handle.getIndex(),
                handle.getConstraint(),
    ...
    public Optional<ConstraintApplicationResult<ConnectorTableHandle>> applyFilter(ConnectorSession session, ConnectorTableHandle table, Constraint constraint) {
        ...
        for (Map.Entry<ColumnHandle, Domain> entry : domains.entrySet()) {
            DatainsightColumnHandle column = (DatainsightColumnHandle) entry.getKey();
        ...
        for (ConnectorExpression expression : expressions) {
            if (expression instanceof Call call) {
                if (isSupportedLikeCall(call)) {
                    ...
                    DatainsightColumnHandle column = (DatainsightColumnHandle) constraint.getAssignments().get(variableName);
                    ...
                    if (!newRegexes.containsKey(columnName) && pattern instanceof Slice) {
                        ...
                        if (metadata.getSchema()
                                .getFields().stream()
                                .anyMatch(field -> columnName.equals(field.getName()) && field.getType() instanceof IndexMetadata.PrimitiveType && "keyword".equals(((IndexMetadata.PrimitiveType) field.getType()).getName()))) {
    ...
    protected static String likeToRegexp(Slice pattern, Optional<Slice> escape) {
        Optional<Character> escapeChar = escape.map(DatainsightMetadata::getEscapeChar);
    ...
    private static boolean isPassthroughQuery(DatainsightTableHandle table) {
        return table.getType().equals(RAW);
    }

    private static boolean isPluginQuery(DatainsightTableHandle table) {
        return table.getType().equals(SQL) || table.getType().equals(PPL);
    }

    @Override
    public Optional<TableFunctionApplicationResult<ConnectorTableHandle>> applyTableFunction(ConnectorSession session, ConnectorTableFunctionHandle handle) {
        if (!(handle instanceof RawQueryFunctionHandle) && !(handle instanceof QueryFunctionHandle)) {
            return Optional.empty();
        }
        if (handle instanceof QueryFunctionHandle) {
            ConnectorTableHandle tableHandle = ((QueryFunctionHandle) handle).getTableHandle();
            List<ColumnHandle> columnHandles = getColumnHandles(session, tableHandle).values().stream().sorted(Comparator.comparing(column -> ((DatainsightColumnHandle) column).getIndex())).toList();
    ...
    public DatainsightRecord getPluginResult(String pluginName, String query) {
        return client.getPluginResult(pluginName, query);
    }

    public Type getColumnType(String type) {
